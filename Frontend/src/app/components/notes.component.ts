...
export class NotesComponent implements OnInit {
	...
	public ngOnInit(): void {
		this.dataShareService.contactSelected$.subscribe(contact => {
			this.selectedContact = contact;
			if(contact.ID) {
				this.noteService.getContactNotes(contact.ID).subscribe(notes => this.notesCollection = notes);
			} else {
	...
	public addNote(contact): void {
		...
		this.noteService.newNote(blankNote, contact.ID).subscribe(() => {
	...
	public removeNote(id: number) {
		this.noteService.deleteNote(id).subscribe(() => {
			...
			this.noteService.getContactNotes(this.selectedContact.ID).subscribe(notes => this.notesCollection = notes);
		});
	}
}
